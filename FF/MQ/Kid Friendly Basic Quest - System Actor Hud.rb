# ╔═════════════════════════════════════╦════════════════════╗
# ║ Title: KFBQ System Actor HUD        ║  Version: 1.00     ║
# ║ Author: Roninator2                  ║                    ║
# ╠═════════════════════════════════════╬════════════════════╣
# ║ Function:                           ║   Date Created     ║
# ║                                     ╠════════════════════╣
# ║   FFMQ Style Actor System HUD       ║    09 Mar 2023     ║
# ╚═════════════════════════════════════╩════════════════════╝
# ╔══════════════════════════════════════════════════════════╗
# ║ Instructions:                                            ║
# ║                                                          ║
...
class Scene_KFBQSystem < Scene_MenuBase
  ...
  # * Start
  #--------------------------------------------------------------------------
  alias r2_options_hud_start  start
  def start
    r2_options_hud_start
    start_hud
  end
  #--------------------------------------------------------------------------
  # * Start HUD
  ...
  # * Update
  #--------------------------------------------------------------------------
  alias r2_options_hud_update    update
  def update
    r2_options_hud_update
    @actor1_hud.update
    if @actor2_hud == nil
      @actor2_hud = Actor_Hud_Window.new($game_party.members[1].id) if $game_party.members[1]
      @actor2_control = Actor_Control_Window.new($game_party.members[1].id) if $game_party.members[1]
  ...
  #--------------------------------------------------------------------------
  # * Select what image to show for HP bar
  #--------------------------------------------------------------------------
  def check_figure
    @figure = $game_system.life_indicator? if @figure.nil?
    return if @figure == $game_system.life_indicator?
    @actor1_hud.refresh if @figure != $game_system.life_indicator?
    @actor2_hud.refresh if @figure != $game_system.life_indicator? && $game_party.members[1]
    @figure = $game_system.life_indicator?
  end
  #--------------------------------------------------------------------------
  ...
  # * Terminate
  #--------------------------------------------------------------------------
  alias r2_terminator_custom_sprite  terminate
  def terminate
    r2_terminator_custom_sprite
    remove_blank_space
  end
  #--------------------------------------------------------------------------
  # * Call for changing values
  #--------------------------------------------------------------------------
  def change_data(value = false)
    update
  end
end
