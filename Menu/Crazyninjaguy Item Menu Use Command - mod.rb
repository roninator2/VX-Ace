...
#===========================================================================
  ...
  Discard   = false
  Details   = false
  Rename    = true  # requires Bubs Rename anything script line 91 
                    # to look like this
                    # $imported["BubsRenameAnything"] = true
	Rename_Size = 20
  Cancel    = true
...
class Window_SubItemCategory < Window_Command
  ...
  def make_command_list
    ...
    add_command("Rename", :rename) if SEE::Item::Rename == true && $imported["BubsRenameAnything"]
...
class Scene_Item < Scene_ItemBase
  ...
  def create_description_window
    @description = Window_ItemDescribe.new
    @description.hide
  ...
  def create_sub_item_window
    ...
    @subitem_window.set_handler(:rename,     method(:on_item_rename))
  ...
  def on_item_rename
    if @item_window.item == nil
      ...
      when RPG::Item 
        ...
        symbol = :item
      when RPG::Armor
        obj = $data_armors[item.id]
        symbol = :armor
      when RPG::Weapon
        obj = $data_weapons[item.id]
        symbol = :weapon
      else
        Sound.play_buzzer
        on_sub_item_cancel
      ...
      if obj
        ...
        SceneManager.scene.prepare(obj, SEE::Item::Rename_Size, symbol)
      end
    end
  end
  def on_item_details
...
class Window_ItemDescribe < Window_Selectable
  include ReadNote
  include SEE::Item
